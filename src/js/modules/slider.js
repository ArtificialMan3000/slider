...
// Слайдер
function slider ({container, slide, nextBtn, prevBtn, totalCounter, currentCounter, wrapper, carousel}) {
    // Элемент слайдера
    const sliderElem = document.querySelector(container);
    // Кнопка "назад"
    const sliderCounterPrevElem = sliderElem ? sliderElem.querySelector(prevBtn) : null;
    // Кнопка "вперёд"
    const sliderCounterNextElem = sliderElem ? sliderElem.querySelector(nextBtn) : null;
